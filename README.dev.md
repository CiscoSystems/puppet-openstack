...
### Networking:
  ...
  the remote management capabilities of the associated [Cobbler Module](https:://github.com/danehans/puppet-cobble).
  Additionally, the [puppet-networking module](https://github.com/danehans/puppet-networking) models OpenStack network configurations.  
...
### Installation Order
  ...
  Deploy your OpenStack Nodes in the following order. For the time being, 
  you need to perform multiple puppet runs for most Nodes to deploy properly.
 ...
 * Swift Proxy Node #1
   Make sure the ring is functional before adding the 2nd Proxy.

 * Swift Proxy Node 2 
   Note: Make sure the ring is functional before proceeding.
	
 * Controller Nodes 1-3 
   Note: You must ensure that the HAproxy Virtual IP address 
